...
`dynamic_api_react_app` is a fullstack starter for projects that want a ready Qornix Dynamic API backend and a React/Vite frontend in one generated application.
...
- a React application served from `/`;
- a React developer dashboard under `/backend-admin`;
...
| Command | Result |
|---------|--------|
| `./create_new_project.sh ../my_react --with-dynamic-api-react` | Create a Dynamic API + React application. |
| `./create_new_project.sh ../my_react --template dynamic-api-react` | Same template through explicit `--template`. |
| `./create_new_project.sh ../my_react --template dynamic_api_react` | Same template using underscore alias. |
| `./create_new_project.sh ../my_react --template dynamic_api_react_app` | Same template using internal template name. |
...
```text
/                         -> React SPA
/backend-admin            -> React developer dashboard
...
Unknown frontend routes are served through the React SPA fallback, while `/api/*`, `/backend/*`, `/static/*`, `/assets/*` and `/health` remain backend/static routes.
...
The generated `vite.config.ts` proxies backend paths to the running C++ backend, so frontend code can use relative requests:
...
```text
build/deploy/my_react/frontend/
```
...
For now, the generated React app is a clean frontend workspace that can be customized freely per project.
