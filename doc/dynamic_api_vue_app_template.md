...
`dynamic_api_vue_app` is a fullstack starter for projects that want a ready Qornix Dynamic API backend and a Vue/Vite frontend in one generated application.
...
- a Vue application served from `/`;
- a Vue developer dashboard under `/backend-admin`;
...
| Command | Result |
|---------|--------|
| `./create_new_project.sh ../my_vue --with-dynamic-api-vue` | Create a Dynamic API + Vue application. |
| `./create_new_project.sh ../my_vue --template dynamic-api-vue` | Same template through explicit `--template`. |
| `./create_new_project.sh ../my_vue --template dynamic_api_vue` | Same template using underscore alias. |
| `./create_new_project.sh ../my_vue --template dynamic_api_vue_app` | Same template using internal template name. |
...
```text
/                         -> Vue SPA
/backend-admin            -> Vue developer dashboard
...
Unknown frontend routes are served through the Vue SPA fallback, while `/api/*`, `/backend/*`, `/static/*`, `/assets/*` and `/health` remain backend/static routes.
...
For now, the generated Vue app is a clean frontend workspace that can be customized freely per project.
