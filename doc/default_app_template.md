...
For the same Dynamic API backend with a Vue/Vite frontend scaffold, use:

```bash
./create_new_project.sh ../my_vue --with-dynamic-api-vue
```

See `dynamic_api_vue_app_template.md` for the Vue template guide.
...
The Dynamic API + Vue application generated by:

```bash
./create_new_project.sh ../my_vue --with-dynamic-api-vue
```

keeps those backend tools under `/backend/*` and serves a Vue/Vite frontend from `/`.
