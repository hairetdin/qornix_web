...
inline void ensureDemoDatabase() {
    ...
    std::call_once(init_flag, []() {
        const auto dbPath = demoDatabasePath();
        ...
        sqlite3* db = nullptr;
        const int rc = sqlite3_open(dbPath.string().c_str(), &db);
        if (rc != SQLITE_OK) {
...
inline DatabaseConfig demoDatabaseConfig() {
    ...
    config.dbname = demoDatabasePath().string();
    config.connectionString = demoDatabasePath().string();
    return config;
}
