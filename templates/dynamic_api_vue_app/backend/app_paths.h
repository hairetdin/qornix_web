...
inline std::filesystem::path frontendDistDir() {
    ...
    // Source-tree development run: CMake builds Vite into frontend/dist,
    ...
    // The deploy bundle does not contain the Vite source tree. Avoid serving
    // the raw source-tree frontend/index.html because it points to /src/main.ts,
    // which only works behind the Vite development server.
