...
inline std::shared_ptr<DynamicAppPageHandler> makeVueSpaHandler() {
    return std::make_shared<DynamicAppPageHandler>(
        (dynamic_api_app_paths::frontendDistDir() / "index.html").string(),
        R"HTML(<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Qornix Dynamic Vue App</title></head><body><h1>Qornix Dynamic Vue App</h1><p>Vue frontend build was not found. Run <code>npm install</code> and <code>npm run build</code> in <code>frontend/</code>, or build the project with CMake.</p><p><a href="/backend">Open backend tools</a></p></body></html>)HTML"
    );
}
