...
inline bool looksLikeAppRoot(const std::filesystem::path& path) {
    std::error_code ec;
    return std::filesystem::exists(path / "templates", ec)
        && std::filesystem::exists(path / "static", ec)
        && std::filesystem::exists(path / "schema", ec);
}
