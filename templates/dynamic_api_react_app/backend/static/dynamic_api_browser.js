...
`;
  ...
  function renderResultTable(payload, tableName = selectedTable()) {
    ...
    target.innerHTML = `<div class="qdb-table-wrap"><table class="qdb-data-table"><thead><tr>${columns.map((col) => `<th>${escapeHtml(col)}</th>`).join('')}<th>Actions</th></tr></thead><tbody>${data.map((row) => `<tr>${columns.map((col) => `<td>${escapeHtml(row[col])}</td>`).join('')}<td>${row.id !== undefined && tableName ? `<a href="/backend/table/${encodeURIComponent(tableName)}/${encodeURIComponent(row.id)}">Open</a>` : ''}</td></tr>`).join('')}</tbody></table></div>`;
  }
  ...
  function parseRowPath() { const parts = window.location.pathname.split('/').filter(Boolean); const offset = parts[0] === 'backend' ? 2 : 1; return { table: decodeURIComponent(parts[offset] || ''), id: decodeURIComponent(parts[offset + 1] || '') }; }
