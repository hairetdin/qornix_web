...
#if defined(@PROJECT_NAME_UPPER@_ENABLE_RAG)
...
#else
        std::filesystem::path path(configValue(flatConfig, "auth.database.path", "data/auth.db"));
        if (path.is_relative()) {
            path = std::filesystem::current_path() / path;
        }
#endif
        std::filesystem::create_directories(std::filesystem::path(path).parent_path());
        config.dbname = std::filesystem::path(path).lexically_normal().string();
