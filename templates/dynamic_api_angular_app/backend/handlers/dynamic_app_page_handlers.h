...
inline std::shared_ptr<DynamicAppPageHandler> makeAngularSpaHandler() {
    return std::make_shared<DynamicAppPageHandler>(
        (dynamic_api_app_paths::frontendDistDir() / "index.html").string(),
        R"HTML(<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Qornix Dynamic Angular App</title></head><body><h1>Qornix Dynamic Angular App</h1><p>Angular frontend build was not found. Run <code>npm install</code> and <code>npm run build</code> in <code>frontend/</code>, or build the project with CMake.</p><p><a href="/backend">Open backend tools</a></p></body></html>)HTML"
    );
}
