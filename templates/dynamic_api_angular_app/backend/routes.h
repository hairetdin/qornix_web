...
inline void setupRoutes(HttpServer& server) {
    ...
    server.add_route("/", makeAngularSpaHandler());
    server.add_route("/backend-admin", makeAngularSpaHandler());
    server.add_route("/project-structure", makeAngularSpaHandler());
    ...
    server.add_route("/health", [](const Request& req, Response& res, const urls::url_view&, const Params&) {
        res = make_json_response(
            http::status::ok,
            req.version(),
            R"({"application":"@PROJECT_NAME@","status":"ok","template":"dynamic_api_angular","path":"/health"})");
    });
    ...
    server.add_route("/{filename}", StaticFileHandler::create(dynamic_api_app_paths::frontendDistDir().string()));
    ...
    server.get_async("/api/dynamic/async-db/health",
        [asyncDatabase, timeout = config.dynamicQueryTimeout](Request req, Url, Params) -> net::awaitable<Response> {
            try {
                ...
                payload["status"] = "ok";
                payload["template"] = "dynamic_api_angular";
                payload["async_db"] = "ok";
